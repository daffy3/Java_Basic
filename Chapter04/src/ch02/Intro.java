...
public class Intro {
    ...
    // clone 메서드
    // - 객체의 원본을 복사하는데 사용하는 메서드
    // - 생성과정의 복잡한 과정을 반복하지 않고 복제할 수 있다.
    // - clone() 메서드를 사용하는 객체의 정보가 동일한 또 다른 인스턴스가 생성되는 것이므로, 객체지향 프로그램에서의 정보은닉, 객체보호의 관점에서 위배될 수 있다.
    // - 해당 클래스의 clone() 메서드의 사용을 허용ㅇ한다는 의미로 cloneable 인터페이스를 명시
}
